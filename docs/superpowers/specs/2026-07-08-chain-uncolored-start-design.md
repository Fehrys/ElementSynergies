...
- **portal:** unchanged rule — at most one portal per path, must be immediately followed by a stone, which sets/resets `activeColor` to that stone's color. This works identically whether the portal is at index 0 or mid-path: no index-0 special-casing is needed, because a portal at index 0 naturally produces a degenerate "before" segment with zero stones and zero specials, which the length filter (see Bug B) drops on its own.
...
The existing segment-splitting logic (one segment if no portal, two if a portal is present, sharing the portal cell as their boundary) is unchanged — it already works in terms of `activeColor` snapshots rather than assuming `path[0]` is a stone.
...
- `tests/core/chain.test.ts`:
  ...
  - New test: chain starting on a portal with two matching stones following — valid, color decided by the first stone, same mechanism as starting on a special tile.
